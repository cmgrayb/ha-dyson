...
class DysonDeviceInfo:
    ...
    def from_raw(cls, raw: dict):
        ...
        # Extract MQTT root topic level if available from cloud API
        mqtt_root_topic_level = raw.get("MqttRootTopicLevel") or raw.get(
    ...
    def get_mqtt_device_type(self) -> str:
        """
        Get MQTT device type using OpenDyson's priority system.

        This replicates OpenDyson's approach for determining the correct MQTT topic root.
        Priority:
        1. mqtt_root_topic_level (direct from cloud API) - preferred
        2. type + variant combination
        3. model analysis fallback
        4. product_type traditional fallback
        """
        import logging

        _LOGGER = logging.getLogger(__name__)

        # Priority 1: Direct MQTT topic from cloud API (OpenDyson's preferred method)
        # Note: This field would come from cloud API if available
        mqtt_root_topic_level = getattr(self, "mqtt_root_topic_level", None)
        if mqtt_root_topic_level:
            _LOGGER.debug(
                f"Using direct MQTT topic from cloud API: {mqtt_root_topic_level}"
            )
            return mqtt_root_topic_level

        # Priority 2: Use variant-aware device type detection
        device_type = self.get_device_type()
        if device_type:
            _LOGGER.debug(f"Using device type from get_device_type(): {device_type}")
            return device_type

        # Priority 3: Fallback to product_type
        _LOGGER.debug(f"Using product_type fallback: {self.product_type}")
        return self.product_type

    def debug_info(self) -> dict:
        """
        Return debug information about MQTT topic determination.

        This helps troubleshoot MQTT topic issues, especially for TP series devices.
        """
        mqtt_topic = self.get_mqtt_device_type()

        # Determine the source of the MQTT topic
        mqtt_root_topic_level = getattr(self, "mqtt_root_topic_level", None)
        if mqtt_root_topic_level:
            source = "mqtt_root_topic_level (cloud API)"
        elif self.get_device_type():
            source = "type+variant mapping"
        else:
            source = "product_type (fallback)"

        return {
            "mqtt_source": source,
            "determined_topic": mqtt_topic,
            "product_type": self.product_type,
            "variant": self.variant,
            "firmware_version": self.version,
            "has_mqtt_root_topic_level": mqtt_root_topic_level is not None,
        }
