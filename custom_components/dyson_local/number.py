...
async def async_setup_entry(
    ...
    if (
        ...
        _LOGGER.debug(
            "Device %s supports oscillation with angle control, adding number entities",
            device.serial,
        )
        entities.extend(
            [
        ...
        _LOGGER.warning(
            "Device %s does not support oscillation with angle control, skipping number entities",
            device.serial,
        )
        _LOGGER.debug(
            "Device attributes: oscillation_angle_low=%s, oscillation_angle_high=%s, enable_oscillation=%s",
            hasattr(device, "oscillation_angle_low"),
    ...
    if entities:
        _LOGGER.debug(
            "Adding %d number entities for device %s", len(entities), device.serial
        )
...
class DysonOscillationLowAngleNumber(DysonEntity, NumberEntity):  # type: ignore[misc]
    ...
    def native_value(self) -> Optional[float]:  # type: ignore[override]
        ...
        if hasattr(self._device, "oscillation_angle_low"):
            return float(self._device.oscillation_angle_low)  # type: ignore[attr-defined]
        return None
    ...
    async def async_set_native_value(self, value: float) -> None:
        ...
        try:
            ...
            await self.hass.async_add_executor_job(
                self._device.enable_oscillation, low_angle, high_angle  # type: ignore[attr-defined]
            )
...
class DysonOscillationHighAngleNumber(DysonEntity, NumberEntity):  # type: ignore[misc]
    ...
    def native_value(self) -> Optional[float]:  # type: ignore[override]
        ...
        if hasattr(self._device, "oscillation_angle_high"):
            return float(self._device.oscillation_angle_high)  # type: ignore[attr-defined]
        return None
...
class DysonOscillationCenterAngleNumber(DysonEntity, NumberEntity):  # type: ignore[misc]
    ...
    def __init__(self, device, name: str):
        ...
        _LOGGER.debug(
            "DysonOscillationCenterAngleNumber entity created for device %s",
            device.serial,
        )
    ...
    def native_value(self) -> Optional[float]:  # type: ignore[override]
        ...
        if hasattr(self._device, "oscillation_angle_low") and hasattr(
            ...
            _LOGGER.debug(
                "native_value accessed for center angle: low=%s, high=%s, center=%s",
                low_angle,
                high_angle,
                center,
            )
            return float(center)
        _LOGGER.debug(
            "native_value accessed but device lacks oscillation angle attributes"
        )
        return None
    ...
    async def async_set_native_value(self, value: float) -> None:
        ...
        _LOGGER.debug(
            "async_set_native_value ENTRY: Setting oscillation center angle to %f for device %s",
            value,
            self._device.serial,
        )
        try:
            ...
            if current_high < current_low:
                _LOGGER.warning(
                    "Invalid current oscillation state: high angle (%d) < low angle (%d)",
                    current_high,
                    current_low,
                )
                return
            ...
            if new_high < new_low:
                _LOGGER.warning(
                    "Cannot set center - would result in invalid oscillation range (high < low)"
                )
                return
            ...
            _LOGGER.debug(
                "Set oscillation center to %d° (range: %d° to %d°)",
                new_center,
                new_low,
                new_high,
            )
    ...
    def available(self) -> bool:  # type: ignore[override]
        """Return True if entity is available."""
        is_available = super().available and hasattr(
            self._device, "oscillation_angle_low"
        )
        _LOGGER.debug(
            "available property for center angle entity: %s (super().available=%s, has_attr=%s)",
            is_available,
            super().available,
            hasattr(self._device, "oscillation_angle_low"),
        )
        return is_available
